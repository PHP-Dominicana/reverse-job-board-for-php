...
<article {{ $attributes->merge([ 'class' => 'my-5 w-full bg-white shadow-sm rounded-md overflow-hidden hover:shadow-xl focus:outline-none focus:bg-gray-50 transition duration-150 ease-in-out']) }}>
    <a href="{{ route('jobs.detail', $job->id)  }}" title="{{ $job->name }}">
        <div class="flex items-center px-4 py-4 sm:px-6">
            <div class="min-w-0 flex-1 flex items-center">
                <div class="flex-shrink-0">
                    <img class="w-24 h-24 rounded-full flex-shrink-0" src="{{ $job->photo_url }}" alt="{{ $job->company_name }}">
                </div>
                <div class="min-w-0 flex-1 pl-4 md:grid md:gap-4">
                    <div>
                        <header>
                            <h2 class="text-xl font-semibold leading-tight text-blue-900 truncate">{{ $job->title }}</h2>
                            <p class="text-sm py-2  font-semibold text-green-600  leading-2  truncate">{{ $job->status }}</p>
                        </header>
                        ...
                        <div class="mt-2 flex items-center text-sm leading-5 text-gray-500">
                            <div class="sm:flex">
                                <div class="mr-6 flex items-center text-sm leading-5 text-gray-500 sm:mt-0">
                                    <x-icons.location-icon class="w-4 h-4 mr-1 flex-shrink-0 fill-gray-500" /> {{ $job->location }}
                                </div>
